...
# ■ 奶奶的AI
#==============================================================================
class Corals_AI < Game_BattlerAI
  #--------------------------------------------------------------------------
  # ○ 物件初始化
  #--------------------------------------------------------------------------
  def initialize(battler)
    ...
    super(battler)
    determine_set
    @ai_trigger = {"atk_cd1" => 0, "atk_cd2" => 0, "skill_cd1" => 0, "skill_cd2" => 0} 
    @lock_run = 0
...
#==============================================================================
  ...
  def can_z?
    ...
    return @dr = 1 if @ai_random > 500 and dist_x < 300
  end
  #-------------------------------------------------------------------------------
  # ○ →Z
  #-------------------------------------------------------------------------------
  def can_fz?
    ...
    return if @target.motion.guarding? and @me.motion.attacking?
    return if @ai_trigger["atk_cd2"] > 0
    return if @me.sp < 30
    return @dr = 1 if @ai_random <= 500
  end
  #-------------------------------------------------------------------------------
  # ○ ↓Z
  #-------------------------------------------------------------------------------
  def can_dz?
    ...
    return if @target.motion.downing?
    return if @target.motion.guarding? and @me.motion.attacking?
    return if @ai_trigger["skill_cd1"] > 0
    return if @me.sp < 30
    return @dr = 1 if (300..600) === @ai_random  
  end
  #-------------------------------------------------------------------------------
  # ○ ↑Z
  #-------------------------------------------------------------------------------
  def can_uz?
    ...
    return if @target.motion.downing?
    return if @target.motion.guarding? and @me.motion.attacking?
    return if @ai_trigger["skill_cd2"] > 0
    return @dr = 1 if (500..800) === @ai_random  
  end
  #-------------------------------------------------------------------------------
  # ○ ←Z
  #-------------------------------------------------------------------------------
  def can_bz?
    ...
    return if @target.motion.guarding? and @me.motion.attacking?
    return true if (100..250) === @ai_random
  end
  #-------------------------------------------------------------------------------
  # ○ X
  #-------------------------------------------------------------------------------
  def can_x?
    ...
    return if @me.motion.cannot_cancel_act?  
    return if @me.motion.on_air?
    return if @target.motion.guarding?
    return if @target.motion.downing?
    return if @ai_level < 2
    return true if (30..135) === dist_x and (0..100) === dist_y and @me.awake_time >= 1200 and !t_well_down?(9) and !t_eva?(9)
  end
  #-------------------------------------------------------------------------------
  # ○ →X
  #-------------------------------------------------------------------------------
...
#==============================================================================
  ...
  def can_fflee?
    ...
    return @dr = 1 if dist_x < 20 and confront?
    return @dr = 2 if @ai_level > 1 and confront?  and dist_x > 260 and @me.awake_time >= 1200 and @ai_random < 320
  end
  #-------------------------------------------------------------------------------
  # ○ 後迴避
  #-------------------------------------------------------------------------------
  ...
  def search_bullet
    for bullet in $scene.battle_bullets
      ...
      if (bullet.x_pos - @me.x_pos).to_i.abs < 110 and rand(11-@ai_level**2) < 1
        case rand(10)
        when 0..7
          @guard_duration = 5
        when 8
  ...
  #  return true if @target.motion.knock_back_duration > 3
   # return @dr = 2 if @ai_level > 1 and !confront? and dist_x > 70 and @me.awake_time >= 1200 #and @ai_random < 320
    return @dr = 1 if dist_x >= 420 and @ai_mode == "move"
    return @dr = 3 if (dist_x < 50 and @ai_random > 800) and confront?#and @me.motion.backedge_dist < 20 and t_confront?
  end
  ...
  def can_walk?
    ...
    if @lock_run > 0
      ...
      return true if (130..666) === dist_x and @ai_mode == "move"
    end
  end
  ...
  def update_value_timecount
    ...
    @ai_trigger["skill_cd1"] -= 1 if @ai_trigger["skill_cd1"] > 0 
    @ai_trigger["skill_cd2"] -= 1 if @ai_trigger["skill_cd2"] > 0 
  ...
  def update_behavior_phase(sign)
    ...
    if $game_switches[STILILA::Corals_FIRST_BATTLE]
      update_behavior_phase2
      return
    end
    
    return if @me.motion.state == "stone_egg"
  ...
  # ○ 更新行為
  #--------------------------------------------------------------------------
  def update_behavior_phase2
  #  @stroll += rand(5)
    # if @stroll % 20 < 7
    #   @me.motion.do_walk(@me.direction)
    #   return
   #  elsif @stroll % 20 >= 7 and @stroll % 20 < 15
   #    @me.motion.do_walk(-@me.direction)
   #   return
   #  end
      
    @me.motion.change_anime("stand") if @me.motion.controllable? and @me.motion.freemove? and @me.motion.state != "stand"
